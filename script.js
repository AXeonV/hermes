...
function buildTextLayout(lines) {
  ...
  let fontGap = fontSize * 0.3;
  ctx.letterSpacing = `${fontGap}px`;
  ...
  let radius = (textWidth * pixelToWorld) / (2 * Math.PI);
  for (let i = 0; i < NPOINTS; i++) {
    let pick = samples[Math.floor(Math.random() * samples.length)];
    let arcX = (pick[0] - cx) * pixelToWorld;
    let y = (cy - pick[1]) * pixelToWorld;
    let rOffset = (Math.random() - 0.5) * thickness;
    let theta = arcX / radius;
    let x = Math.sin(theta) * (radius + rOffset);
    let z = Math.cos(theta) * (radius + rOffset);
...
window.addEventListener("keydown", (e) => {
  ...
  if (e.code === "Space" && !e.repeat) {
    ...
    if (elapsed < TIMING.t1) {
      ...
      let dis = TIMING.dur - (elapsed - TIMING.t3);
      if (dis < 0) dis = 0;
      TIMING.t3 = elapsed - dis;
...
function transfer(elapsed, start, duration, is_t3) {
  let t = (elapsed - start) / duration;
  t = THREE.MathUtils.clamp(t, 0, 1);
  t = t * 2 + (is_t3 && TIMING.flip ? 1 : -1);
  return (Math.sin(t * Math.PI * 0.5) + 1) * 0.5;
}
...
renderer.setAnimationLoop(() => {
  ...
  controls.update();
  let elapsed = timer.getElapsed();
  gu.morphA.value = transfer(elapsed, TIMING.t1, TIMING.dur, 0);
  gu.morphB.value = transfer(elapsed, TIMING.t2, TIMING.dur, 0);
  gu.morphC.value = transfer(elapsed, TIMING.t3, TIMING.dur, 1);
